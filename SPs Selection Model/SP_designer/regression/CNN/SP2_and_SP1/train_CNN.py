...
parser.add_argument('--model_path', metavar='DIR',
                    help='', default="D:/SP_designer/regression/CNN/SP2_and_SP1/CharCNN_best.pth.tar")
...
parser.add_argument('--saved_values_path', metavar='DIR',
                    help='saved_values', default="D:/SP_designer/regression/CNN/SP2_and_SP1/saved_values.xlsx")
...
learn.add_argument('--batch_size', type=int, default=49, help='batch size for training [default: 64]')
...
experiment.add_argument('--save_folder', default='D:/SP_designer/regression/CNN/SP2_and_SP1',
...
def main():
    ...
    for fold, (_, valid_index) in enumerate(kf.split(test_set)):
        ...
        # 模型初始化
        model = CharCNN(args).to(args.device)
        ...
        spearmanr_corrs.append(spearman_corr)
    ...
    avg_spearmanr=np.mean(spearmanr_corrs)
    avg_r_squared = np.mean(r_squareds)
    avg_rmse = np.mean(rmses)

    print(f"平均Pearson Correlation: {avg_pearson:.2f}")
    print(f"平均Spearman Correlation: {avg_spearmanr:.2f}")
