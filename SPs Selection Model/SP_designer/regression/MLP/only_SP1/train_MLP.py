...
from regression.MLP.model_MLP import CharMLP
...
parser.add_argument('--model_path', metavar='DIR',
                    help='', default="D:/SP_designer/regression/MLP/only_SP1/CharMLP_best.pth.tar")
...
parser.add_argument('--saved_values_path', metavar='DIR',
                    help='saved_values', default="D:/SP_designer/regression/MLP/only_SP1/saved_values.xlsx")
...
learn.add_argument('--batch_size', type=int, default=134, help='batch size for training [default: 64]')
...
parser.add_argument("--T_max", type=int, default=45, help="maximum number of iterations for CosineAnnealingLR")

# model (MLP regressor)
mlp = parser.add_argument_group('Model options')
mlp.add_argument('--dropout', type=float, default=0.3, help='the probability for dropout [default: 0.5]')

# experiment options
experiment = parser.add_argument_group('Experiment options')
experiment.add_argument('--checkpoint', dest='checkpoint', default=True, action='store_true',
                        help='Enables checkpoint saving of model')
experiment.add_argument('--save_folder', default='D:/SP_designer/regression/MLP/only_SP1',
...
def train(train_dataloader, valid_dataloader, model, optimizer, scheduler, args):
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if best_loss is None or val_loss < best_loss:
            file_path = '%s/CharMLP_best.pth.tar' % (args.save_folder)
...
def main():
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(dataset)):
        ...
        # 模型初始化
        model = CharMLP(args).to(args.device)
