...
from classification.Transformer.model_Transformer import CharTransformer
from classification.dataset import AJSDataset
# matplotlib.use('TkAgg')

# end of edits for google bolab compatibility and local environment

parser = argparse.ArgumentParser(description='Transformer classifier training')
# data
...
parser.add_argument('--model_path', metavar='DIR',
                    help='', default="D:/SP_designer/classification/Transformer/SP2_trans_SP1/CharTransformer_best.pth.tar")
...
learn.add_argument('--batch_size', type=int, default=50, help='batch size for training [default: 64]')
...
parser.add_argument("--lr-min", type=float, default=0.001, help="minimum learning rate for CosineAnnealingLR")
parser.add_argument("--T-max", type=int, default=57, help="maximum number of iterations for CosineAnnealingLR")

# model (text classifier)
Transformer = parser.add_argument_group('Model options')
Transformer.add_argument('--num_classes', type=int, default=2, help='')
Transformer.add_argument('--num_encoder_layers', type=int, default=2, help='')
Transformer.add_argument('--hidden_dim', type=int, default=12, help='')
Transformer.add_argument('--num_heads', type=int, default=4, help='')

# experiment options
experiment = parser.add_argument_group('Experiment options')
experiment.add_argument('--checkpoint', dest='checkpoint', default=True, action='store_true',
                        help='Enables checkpoint saving of model')
experiment.add_argument('--save_folder', default='D:/SP_designer/classification/Transformer/SP2_trans_SP1',
...
def train_and_evaluate(model, train_loader, valid_loader, optimizer, scheduler, args):
    ...
    best_model = CharTransformer(args)
...
def train(train_dataloader, valid_dataloader, model, optimizer, scheduler, args):
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if best_acc is None or val_acc > best_acc:
            file_path = '%s/CharTransformer_best.pth.tar' % (args.save_folder)
...
def main():
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(dataset)):
        ...
        # 模型初始化
        model = CharTransformer(args).to(args.device)
        ...
        # 调用训练和验证
        accuracy, precision, recall, f1, predictions, true_labels= train_and_evaluate(model, Train_dataloader, Valid_dataloader, optimizer, scheduler, args)
    ...
    # 绘制 ROC 曲线
    fpr, tpr, _ = roc_curve(all_true_labels, all_predictions, pos_label=1)
