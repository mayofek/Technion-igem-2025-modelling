...
from regression.CNN.model_CNN import CharCNN
...
# target_list=[0 for i in range(len(seq_list))]
# temp_data = pd.DataFrame(
#     {'seq': seq_list,'target':target_list})
# temp_data.to_excel('./temp_data.xlsx', index=False)

# NEW: These will be moved to main() to use command line arguments


parser = argparse.ArgumentParser(description='CNN regression training')
# data
...
parser.add_argument('--model_path', metavar='DIR',
                    help='', default="D:/SP_designer/prediction/fine_CharCNN_best_reg.pth.tar")
...
parser.add_argument('--sampling_path', metavar='DIR',
                    help='path to sampling data for normalization', default=r'D:/SP_designer/data/SP1.xlsx')

# COMMENTED OUT: Original hardcoded sampling path
# sampling_path='D:/SP_designer/data/SP1.xlsx'
# sampling_data = pd.read_excel(sampling_path)
# sampling_std = sampling_data['experimental yield'].std()
# sampling_mean = sampling_data['experimental yield'].mean()

# NEW: This will be moved to main() to use command line arguments
...
def test(dataloader, model, sampling_std, sampling_mean, args):
    res_list = []
    model.eval()
    for i_batch, (data) in enumerate(dataloader):
        ...
        output=output*sampling_std+sampling_mean
...
def main():
    ...
    # NEW: Load sampling data for normalization using command line argument
    sampling_data = pd.read_excel(args.sampling_path)
    sampling_std = sampling_data['experimental yield'].std()
    sampling_mean = sampling_data['experimental yield'].mean()
    ...
    res_list=test(dataloader, model, sampling_std, sampling_mean, args)

    # COMMENTED OUT: Original hardcoded output
    # save_data=pd.read_excel(source_path)
    # save_data['预测酶活']=res_list
